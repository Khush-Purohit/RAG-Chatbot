...
def enforce_sql_safety(sql: str) -> str:
    ...
    # Remove markdown code block if present (e.g., ```sql ... ```)
    sql = re.sub(r"```sql\s*\n?", "", sql, flags=re.I)
    sql = re.sub(r"\n?```", "", sql)
    sql = sql.strip()
...
def generate_sql_with_llm(
    ...
    """
    Generate SQL using local Ollama with optional conversation context.
    ...
    # Build SQL prompt
    full_prompt = f"""
You are a SQL generator. Convert the user's request into a valid SQLite SELECT query.
...
"""
    ...
    try:
        response = ollama_client.generate(model=ollama_model, prompt=full_prompt)
